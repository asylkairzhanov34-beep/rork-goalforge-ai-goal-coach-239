            ...
            { text: 'Cancel', style: 'cancel', onPress: () => setTapCount(0) },
            {
              ...
              onPress: async () => {
                try {
                  ...
                  await AsyncStorage.setItem(TRIAL_START_KEY, newTrialStart);
                  await AsyncStorage.removeItem(BLOCK_PERSISTED_KEY);
                  await AsyncStorage.removeItem('trialStartedAt');
                  console.log('[DevReset] Trial reset to:', newTrialStart);
                  Alert.alert('✅ Done', 'Trial reset. Please restart the app.', [
                    { text: 'OK' }
                  ]);
                } catch (e) {
        ...
        <Animated.View style={[styles.card, { opacity: fade, transform: [{ scale }] }]}>
          ...
          >
            ...
            {tapCount > 0 && tapCount < 5 && (
              <Text style={styles.devHint}>{5 - tapCount} taps left</Text>
            )}
          </TouchableOpacity>
